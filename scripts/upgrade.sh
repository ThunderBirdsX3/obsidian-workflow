...
# Load config (best-effort — resolver may fail-closed if yq absent; TEST_ENV_FILE
...
# configured test-credentials env file.
SAFE_PATHS=()
while IFS= read -r _sp; do [ -n "$_sp" ] && SAFE_PATHS+=("$_sp"); done < <(ow_safe_paths "${TEST_ENV_FILE:-}")
...
# Back up .ow.yml PRISTINE (before the config-block backfill below)
# so --rollback restores the exact pre-upgrade config. It is otherwise protected/untouched.
...
# Retire the per-machine config template: config lives in .ow.yml only. Version-gated,
# backed up first, listed in ROLLBACK_PATHS.
if command -v _ow_ver_lt >/dev/null 2>&1 && _ow_ver_lt "$current_version" "1.2.0" && [ -f .ow.local.yml.example ]; then
  cp .ow.local.yml.example "$BACKUP_DIR/.ow.local.yml.example"
  rm -f .ow.local.yml.example
  warn "Retired .ow.local.yml.example (per-machine config removed, v1.2.0) — backed up in $BACKUP_DIR"
fi
...
# A personal .ow.local.yml is not read by anything. It is the user's file, so it is left in
# place — only reported, so a value still sitting in it can be moved into .ow.yml.
if [ -f .ow.local.yml ]; then
  warn ".ow.local.yml is not read — move any value you still need into .ow.yml, then delete it"
fi
